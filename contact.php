...
$SMTP_USER   = 'growthpixelagency@gmail.com';        // ← CHANGE
$SMTP_PASS   = 'huty lgpa pdgc gjnj';   // ← CHANGE

$FROM_EMAIL  = $SMTP_USER;         // Gmail rewrites From to the authenticated user anyway
$FROM_NAME   = 'Website Enquiry';


// ── Pabbly webhook (for WhatsApp) — paste your Pabbly Connect URL here ──
$PABBLY_WEBHOOK = 'https://connect.pabbly.com/webhook-listener/webhook/IjU3NjcwNTZlMDYzMDA0MzU1MjZlNTUzZCI_3D_pc/IjU3NjcwNTY5MDYzMjA0MzU1MjZhNTUzMTUxMzIi_pc';
...
$RECAPTCHA_SECRET     = '6Lfgj0ctAAAAAEgcbmdt7GYtK78jwarmD_0ExYZt';    // ← paste v3 SECRET key to turn on
...
// ── Forward the enquiry to Pabbly (→ WhatsApp) ───────────────────────────
if (!empty($PABBLY_WEBHOOK)) {
    $payload = json_encode([
        'name'    => $name,
        'phone'   => $phone,
        'email'   => $email,
    ...
    if (function_exists('curl_init')) {
        ...
        @file_get_contents($PABBLY_WEBHOOK, false, stream_context_create([
            'http' => [
                'method'  => 'POST',
                'header'  => "Content-Type: application/json\r\n",
                'content' => $payload,
